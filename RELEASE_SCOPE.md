...
| Message omission, duplication, reordering, and partition | Deterministic transport faults and local Jepsen loopback profiles; a healthy quorum side must remain. | `epaxos/dst_test.go`, `epaxos/stress_test.go`, `TestCheckpointPartitionHealRetriesVoteAndConverges`, and the local campaign cover heal-and-converge and minority fail-closed behavior. | No real-network history. |
| Logical timing faults | Deterministic `RawNode.Tick` skew and pause; explicit TOQ uses caller-provided samples and configured delay bounds. | `TestUnevenLogicalTickSkewAndBurstConvergesWithoutDuplicates`, `TestPausedClockDoesNotTickOrProcessReadyUntilResume`, and `tla/TOQClockDiscipline.cfg`. | The core does not sample or synchronize clocks or measure one-way delay. |
...
| Certified checkpoint and protocol compaction | Three-replica bounded execution with exact frontiers, prepared snapshot durability, slow-quorum certification, atomic tombstone/delete, restart from snapshot plus delta, and stale traffic. | Core/KV checkpoint tests, `TestCheckpointPartitionHealRetriesVoteAndConverges`, `TestCheckpointCompactionThreeReplicaRestartSmoke`, and `tla/EPaxosCertifiedCompaction.tla` fast/full configs. | No arbitrary history, unbounded refinement, quorum-loss recovery, or target-environment claim. |
...
| Certified protocol-state compaction | `epaxos/checkpoint*.go`, Pebble v9/checkpoint storage, content-addressed snapshot transfer, focused crash/certificate/restart tests, the three-replica smoke, and finite `EPaxosCertifiedCompaction` TLC profiles. |
...
- `tla/EPaxosVoterBootstrap.tla` is a finite bootstrap contract executed by the 32-job TLC fast gate for base sizes 1–6 (successor sizes 2–7); bootstrap state is still outside the sampled Go-to-TLC replay.
