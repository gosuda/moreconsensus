...
| Configuration-change ordering and pinning | Finite local barrier plus one add-voter, one remove-voter, and one add-then-remove configuration chain. | `tla/EPaxosConfigBarrier.cfg`; `tla/EPaxosConfigTransition.cfg`; `tla/EPaxosConfigRemoveTransition.cfg`; `tla/EPaxosConfigChainTransition.cfg`; `go test ./epaxos`; no arbitrary/unbounded/recovery-under-config-change proof. |
