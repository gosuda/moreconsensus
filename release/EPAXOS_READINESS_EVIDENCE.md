...
| Configuration-change ordering and pinning | Finite local barrier plus one add-voter transition. | `tla/EPaxosConfigBarrier.cfg`; `tla/EPaxosConfigTransition.cfg`; `go test ./epaxos`; no arbitrary/remove-voter/multi-step/recovery-under-config-change proof. |
...
- `tla/EPaxosConfigTransition.tla` and `tla/EPaxosConfigTransition.cfg`
  - Add finite add-voter configuration-transition coverage: a config command chosen/executed under the old config, an old in-flight user instance remaining pinned to old voters/quorum after transition, and a later user instance using the new voters/quorum. This does not model remove-voter transitions, multi-step reconfiguration chains, joint consensus, recovery under configuration changes, durable replay, or unbounded configuration histories.
...
- `epaxos/sim_test.go`, `epaxos/recovery_test.go`, `epaxos/remaining_test.go`, `epaxos/optimized_test.go`, and `epaxos/branch_test.go`
  - Cover the updated quorum table, five-node optimized fast path, divergent slow-path fallback, timing-boundary behavior, optimized TryPreAccept witness thresholds, rollback catch-up local `nextInstance` monotonicity, learned-before-fresh applied-order assertion, restart recovery timers, stale accept-reject hints, stale-originator fast-commit prerequisites, and local/remote fast-path eligibility evidence.
- `epaxos/dst_test.go`
