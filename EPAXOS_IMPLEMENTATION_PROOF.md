...
This document explains the EPaxos and EPaxos Revisited behavior implemented in this repository, why each stated property is expected to hold inside the documented fault envelope, and which evidence currently supports the claim. It is intentionally proof-oriented. It does not replace `RELEASE_SCOPE.md`: the current release decision remains **no-go** until every open release item and verification prerequisite has current evidence.
...
- CMU-PDL-13-111 Section 6.2 contains a full optimized-recovery decision tree. This repository implements the Go committed stale-dependency evidence-search/resend-ignore path for supported `F <= 3` configurations, ballot-scoped evidence response handling, durable value-ballot recovery evidence, targeted leader-in-fast-quorum/deferred-cycle handling, finite TLA coverage for the committed-conflict evidence-query plus evidence-staleness guard/fail-closed slices, and `tla/EPaxosOptimizedRecoveryDecisionTree.tla` coverage for every implemented F<=3 Accept-Deps decision branch. Unbounded proof, arbitrary networks, arbitrary message loss/retry schedules, and recovery under reconfiguration remain non-claims.
- `tla/EPaxosTryPreAcceptRetry.tla` adds a finite logical timer slice for the TryPreAccept retry path after pending-evidence checks, including current-evidence fail-closed timeout, stale-evidence cleanup, unrelated-evidence retention, normal-vs-ignore rebroadcast separation, and no durable/application effects on pure retry. It does not model arbitrary network delivery, message loss, recovery under reconfiguration, or unbounded proof.
...
Additional source anchors for finite optimized-recovery decision-tree branch parity: `tla/EPaxosOptimizedRecoveryDecisionTree.tla`, `tla/EPaxosOptimizedRecoveryDecisionTree.cfg`, `epaxos/protocol_coverage_test.go`, and `epaxos/optimized_test.go`.
...
- `tla/EPaxosConfigChainTransitionRetry.tla` checks one finite add-then-remove transition retry slice after `{1,2,3}` adds voter 4 and then removes voter 2: old Conf1 local-owner PreAccept/Accept retries after current Conf3 stay pinned to peers `{2,3}` and width-3 dependencies, mid Conf2 retries stay pinned to peers `{2,3,4}` and width-4 dependencies including removed voter 2, and pure retries have no durable/application effects. The matching Go regression is `TestOldConfigChainTransitionRetryUsesPinnedVotersAfterAddThenRemove`.
- `tla/EPaxosConfigChainTransitionLostResponseRetry.tla` checks one finite add-then-remove transition lost-response slice: after current Conf3, old Conf1 local-owner PreAccept/Accept rejects/non-counts current voter 4 before retry, then advances only when removed voter 2 responds after retry to the old `{2,3}` peer set with width-3 dependencies; mid Conf2 counts voter 4 below the old 3/4 quorum, then advances only when removed voter 2 responds after retry to `{2,3,4}` with width-4 dependencies. The matching Go regression is `TestOldConfigChainTransitionRetryCompletesAfterLostPreRetryResponses`.
...
- `tla/EPaxosConfigTransitionLostResponseRetry.tla` checks one finite explicit lost-response-before-retry slice for normal local-owner old instances after both removal and addition transitions: in the removal PreAccept/Accept slices, local owner 1 plus surviving voter 2 is still below the old 3/4 quorum despite being a current-config quorum, removed voter 4 responses are explicit pre-retry losses, deterministic retries stay pinned to old peers `{2,3,4}` and width-4 dependencies, and replacement removed-voter responses advance through the old quorum; in the addition PreAccept/Accept slices, added voter 4 is excluded, old voter 2 responses are explicit pre-retry losses, retries target old peers `{2,3}` with width-3 dependencies, and replacement old-voter responses advance through the old 2/3 quorum. The matching Go regression is `TestOldConfigTransitionRetryCompletesAfterLostPreRetryResponses`.
...
Additional source anchors for finite config-transition lost-response retry coverage: `tla/EPaxosConfigTransitionLostResponseRetry.tla`, `tla/EPaxosConfigTransitionLostResponseRetry.cfg`, and `epaxos/recovery_test.go` (`TestOldConfigTransitionRetryCompletesAfterLostPreRetryResponses`).
...
Additional source anchors for finite mid-chain config-recovery lost-response retry coverage: `tla/EPaxosConfigChainRecoveryLostResponseRetry.tla`, `tla/EPaxosConfigChainRecoveryLostResponseRetry.cfg`, and `epaxos/recovery_test.go` (`TestOldConfigChainRecoveryRetryCompletesAfterLostPreRetryResponses`).
Additional source anchors for finite config-chain transition retry coverage: `tla/EPaxosConfigChainTransitionRetry.tla`, `tla/EPaxosConfigChainTransitionRetry.cfg`, and `epaxos/recovery_test.go` (`TestOldConfigChainTransitionRetryUsesPinnedVotersAfterAddThenRemove`).
Additional source anchors for finite config-chain transition lost-response retry coverage: `tla/EPaxosConfigChainTransitionLostResponseRetry.tla`, `tla/EPaxosConfigChainTransitionLostResponseRetry.cfg`, and `epaxos/recovery_test.go` (`TestOldConfigChainTransitionRetryCompletesAfterLostPreRetryResponses`).
...
| `tla/EPaxosOptimizedRecovery.tla` | Focused 3/5/7 Accept-Deps stale-dependency optimized-recovery evidence slices. | The concrete `MsgEvidence` exchange, arbitrary networks, arbitrary message loss/retry histories, and unbounded proof. |
| `tla/EPaxosTryPreAcceptBranches.tla` | Focused 3/5/7 finite abstract TryPreAccept scenario/stage response-branch slice: stale restart, committed evidence ignore/fail-closed, direct/forced accept, one uncommitted deferral with duplicate suppression, and OK slow-quorum accept with `okVotes >= SlowQuorum` as the only quorum detail in this model. | TryPreAccept message paths, unbounded recovery trees, arbitrary message loss, and recovery under reconfiguration. |
| `tla/EPaxosTryPreAcceptMessagePath.tla` | Focused 3/5/7 finite TryPreAccept request/response message paths: follower commit-only, stale/conflict reject, duplicate matching re-ack without durable rewrite, fresh durable ack; coordinator stale restart, committed evidence/direct accept, uncommitted forced/deferred handling, older-ballot ignore, duplicate OK ignore, first OK below quorum, pre-seeded quorum immediate accept, and OK slow-quorum accept. | Full network histories, evidence-query internals, unbounded recovery trees, arbitrary message loss, and recovery under reconfiguration. |
| `tla/EPaxosTryConflictForce.tla` | Focused 3/5/7 finite quorum-arithmetic check for `tryConflictForcesSlowAccept`: required conflict leader forces slow Accept only without an existing dependency, required deferred-cycle leader forces slow Accept, optional leaders defer, and non-force cases start blocker recovery. | Full recovery histories, TryPreAccept message paths, evidence-query internals, arbitrary message loss, and unbounded proof. |
| `tla/EPaxosEvidenceQuery.tla` | Focused 3/5/7 committed-conflict evidence-query slice: guard-gated `MsgEvidence`, read-only responses, duplicate/mismatched drops, sender-preserving evidence validation, stale rejection restart, and fail-closed fallback. | Arbitrary network histories, arbitrary membership/reconfiguration recovery, and unbounded proof. |
| `tla/EPaxosTryPreAcceptRetry.tla` | Focused finite TryPreAccept retry timer: current-ballot same-target pending evidence fails closed before retry, stale same-target evidence is deleted, unrelated-target evidence is retained, and normal or ignore-marker retry rebroadcasts have no durable/application effects. | Arbitrary network delivery, message loss, recovery under reconfiguration, and unbounded proof. |
| `tla/EPaxosOptimizedRecoveryDecisionTree.tla` | Focused finite F<=3 Accept-Deps decision table for every implemented CMU-PDL-13-111 Section 6.2 branch: no-information no-op, committed commit, accepted/preaccepted slow Accept, committed-conflict evidence authorize/fail-closed/direct accept, uncommitted force/defer, stale restart, OK quorum, and duplicate/old response ignore. | Arbitrary networks, arbitrary message loss/retry schedules, recovery under reconfiguration, arbitrary durable histories, and unbounded recovery trees. |
...
| `tla/EPaxosConfigChainTransitionRetry.tla` | One finite add-then-remove normal transition retry-timer slice where old Conf1 local-owner PreAccept/Accept retries after current Conf3 target `{2,3}` with width-3 dependencies, mid Conf2 retries target `{2,3,4}` with width-4 dependencies including removed voter 2, and pure retries have no durable/application effects. | Arbitrary retry histories beyond this timer slice, message loss, joint consensus, arbitrary membership histories, durable replay, application output, and unbounded proof. |
| `tla/EPaxosConfigChainTransitionLostResponseRetry.tla` | One finite add-then-remove normal transition lost-response-before-retry slice where old Conf1 local-owner PreAccept/Accept rejects/non-counts current voter 4 before retry and advances only after replacement removed voter 2 with width-3 dependencies, while mid Conf2 counts current voter 4 below the old 3/4 quorum and advances only after replacement removed voter 2 with width-4 dependencies. | Arbitrary message loss beyond this named finite pre-retry loss shape, arbitrary retry histories, joint consensus, arbitrary membership histories, durable replay, application output beyond the modeled command, and unbounded proof. |
...
| `tla/EPaxosConfigTransitionLostResponseRetry.tla` | One finite normal old-config transition lost-response retry slice where local-owner PreAccept/Accept responses after removal and addition are not counted before deterministic retry; removal keeps old 3/4 quorum despite a current-config quorum, addition excludes the added current voter, retry targets and dependency widths remain old-pinned, and replacement responses after retry advance only through the old pinned quorum. | Arbitrary retry histories beyond this timer slice, arbitrary message loss beyond this named finite pre-retry loss shape, joint consensus, arbitrary membership histories, application output beyond the modeled command, and unbounded proof. |
...
Evidence consists of focused checkpoint crash/certificate/restart tests, a three-replica Pebble smoke, tagged KV behavior and race tests, deterministic fault simulation, bounded finite model checking including `EPaxosCertifiedCompaction`, the five-control sampled Go-to-TLC replay, and partial TLAPS checking with nine failed obligations. Deployment/service evidence is limited to Darwin arm64, three-node same-host loopback exercises, and simulated network faults. It does not provide an arbitrary-Go refinement theorem, arbitrary compaction or crash/network histories, independent producer/reviewer signatures, native-Darwin target attestation, or a mission-critical production-readiness claim.
