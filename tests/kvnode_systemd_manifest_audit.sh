...
write_manifest_report() {
  ...
  local required_vars_csv
  required_vars_csv="$(IFS=,; printf '%s' "${required_vars[*]}")"
  ...
  mkdir -p "$(dirname "$report_path")"
  {
    ...
    echo "node_id=$KVNODE_ID"
    echo "peer_count=${#peers[@]}"
    printf 'required_env_vars=%s\n' "$required_vars_csv"
    echo "exec_contract=env-file-rendered"
    printf 'deadline_defaults=request_deadline_ms=%s,peer_deadline_ms=%s\n' "$KVNODE_REQUEST_DEADLINE_MS" "$KVNODE_PEER_DEADLINE_MS"
    printf 'body_scan_limits=max_client_body_bytes=%s,max_peer_body_bytes=%s,max_admin_body_bytes=%s,max_scan_limit=%s\n' "$KVNODE_MAX_CLIENT_BODY_BYTES" "$KVNODE_MAX_PEER_BODY_BYTES" "$KVNODE_MAX_ADMIN_BODY_BYTES" "$KVNODE_MAX_SCAN_LIMIT"
    echo "hardening=User,Group,StateDirectory,ProtectSystem,NoNewPrivileges"
    echo "evidence_files=deploy/systemd/kvnode@.service,deploy/systemd/kvnode.env.example"
