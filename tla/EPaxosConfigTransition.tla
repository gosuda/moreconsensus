---- MODULE EPaxosConfigTransition ----
EXTENDS Naturals, FiniteSets

(***************************************************************************)
(* Focused finite configuration-transition model.  It checks one old       *)
(* three-voter configuration, one chosen/executed configuration command    *)
(* that installs a four-voter successor, one user instance that was        *)
(* already in flight under the old configuration, and one later user       *)
(* instance under the new configuration.  It models config-id/voter-set    *)
(* pinning and quorum derivation per instance; it does not model joint     *)
(* consensus, concurrent configuration commands, recovery, message loss,   *)
(* durable replay, or unbounded configuration histories.                   *)
(***************************************************************************)
...
OldVoters == {1, 2, 3}
AddedVoter == 4
NewVoters == OldVoters \cup {AddedVoter}
...
ChooseOldUser(q) ==
    ...
    /\ q \in QuorumSets(pinnedConf[OldUser])
    /\ stage' = 4
...
Next ==
    ...
    \/ ExecuteConfig
    \/ \E q \in QuorumSets(OldConf) : ChooseOldUser(q)
    \/ StartNewUser
...
OldInFlightInstanceRemainsPinnedToOldConfig ==
    status[OldUser] # "none" =>
        ...
        /\ (status[OldUser] = "chosen" => AddedVoter \notin quorumVotes[OldUser])
...
EventuallyCoversConfigTransition ==
    <> ( /\ stage = 6
         /\ currentConf = NewConf
         /\ status[ConfigCmd] = "executed"
         /\ status[OldUser] = "chosen"
...
Spec ==
    ...
    /\ \A q \in QuorumSets(OldConf) : WF_Vars(ChooseOldUser(q))
