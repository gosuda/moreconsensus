...
- [tla/EPaxosAcceptEvidenceMerge.tla](tla/EPaxosAcceptEvidenceMerge.tla) contains the finite AcceptEvidence sender-merge and validation model for zero-sender skipping, same-sender coalescing, and duplicate-conflict rejection.
...
- [tla/EPaxosConfigRecoveryLostResponseRetry.tla](tla/EPaxosConfigRecoveryLostResponseRetry.tla) contains the finite configuration-recovery model for one explicit old-config lost response before deterministic retry, without claiming arbitrary message-loss retry behavior.
